...
// Plan generation now ONLY accepts an existing clientId
// All client data is fetched from the database - client record is the single source of truth
...
export const createClientSchema = z.object({
  ...
  email: sanitizedString
    ...
    .optional()
    .or(z.literal('')),
  phone: sanitizedString.pipe(z.string().max(20)).optional().or(z.literal('')),
...
export type ValidatedClientInput = z.infer<typeof createClientSchema>

export const updateClientSchema = z.object({
  name: sanitizedString.pipe(z.string().min(1, 'Client name is required').max(100)).optional(),
  email: sanitizedString
    .pipe(z.string().email('Invalid email address'))
    .optional()
    .or(z.literal('')),
  phone: sanitizedString.pipe(z.string().max(20)).optional().or(z.literal('')),
  age: z.coerce.number().int().min(16, 'Age must be at least 16').max(100, 'Age must be 100 or less').optional(),
  gender: z.enum(['M', 'F'], { errorMap: () => ({ message: 'Gender must be M or F' }) }).optional(),
  height: z.coerce.number().min(140, 'Height must be at least 140cm').max(220, 'Height must be 220cm or less').optional(),
  weight: z.coerce.number().min(40, 'Weight must be at least 40kg').max(200, 'Weight must be 200kg or less').optional(),
  ideal_weight: z.coerce.number().min(40, 'Goal weight must be at least 40kg').max(200, 'Goal weight must be 200kg or less').optional(),
  activity_level: z.enum(
    ['sedentary', 'lightly_active', 'moderately_active', 'very_active', 'extremely_active'],
    { errorMap: () => ({ message: 'Invalid activity level' }) }
  ).optional(),
  ...
  allergies: sanitizedString.pipe(z.string().max(500)).optional(),
  dislikes: sanitizedString.pipe(z.string().max(500)).optional(),
  preferences: sanitizedString.pipe(z.string().max(500)).optional(),
  budget: z.coerce.number().int().min(10, 'Budget must be at least £10').max(1000, 'Budget must be £1000 or less').optional(),
  cooking_skill: z.enum(
    ['beginner', 'intermediate', 'advanced'],
    { errorMap: () => ({ message: 'Invalid cooking skill level' }) }
  ).optional(),
  prep_time: z.coerce.number().int().min(10, 'Prep time must be at least 10 minutes').max(120, 'Prep time must be 120 minutes or less').optional(),
  meals_per_day: z.coerce.number().int().min(2, 'Must be at least 2 meals').max(6, 'Must be 6 meals or less').optional(),
  plan_duration: z.coerce.number().int().min(3, 'Plan must be at least 3 days').max(30, 'Plan must be 30 days or less').optional(),
  meal_prep_style: z.enum(
    ['daily', 'batch', 'mixed'],
    { errorMap: () => ({ message: 'Invalid meal variety option' }) }
  ).optional(),
